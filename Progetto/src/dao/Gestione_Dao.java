 ...
 */   
      ...
      int count = 0;  // Contatore per il numero effettivo di elementi inseriti      
              ...
              try (PreparedStatement statement = conn.prepareStatement(sql)) {
                  ...
                  if (!trovateStanze) {
                	  ...
                	  mostrastanzeingui(numeriCamera, count, tabella);
                      return 0;
                  }
              }
  }
 ...
 public int effettuaprenotazione(String datain, String dataout, String selectedValueCol1, String selectedValueCol3, int id) throws SQLException {
	 DBConnessione d = new DBConnessione();
     conn = d.connessione(conn);
     ...
     int costosingolo = Integer.parseInt(selectedValueCol3);
     int costototale = datetotali * costosingolo;
     LocalDate datapr = LocalDate.now();
     Date dataprsql = Date.valueOf(datapr);
     int numerocamera = Integer.parseInt(selectedValueCol1);
     
     try (PreparedStatement statement = conn.prepareStatement(sql)){
    	 statement.setInt(1, codice);
    	 statement.setString(2, datain);
    	 statement.setString(3, dataout);
    	 statement.setInt(4, costototale);
    	 statement.setDate(5, dataprsql);
    	 statement.setInt(6, numerocamera);
...
public void MostraLePrenotazioni(int id, JTable table) throws SQLException {
	    ...
	    try (PreparedStatement stmt = conn.prepareStatement(query)) {
	        ...
	        while (rs.next()) {
	            ...
	            System.out.println("" + giorno + numero + id2);
